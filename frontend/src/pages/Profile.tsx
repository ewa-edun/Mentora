...
import { UserCircle, Mail, Calendar, Edit3, Save, X, Trash2, Sparkles, Settings, Shield, Bell, Mic, Heart, BookOpen, Trophy, Clock, ArrowLeft, AlertTriangle } from 'lucide-react';
import { getCurrentUser, getUserProfile, updateUserProfile, logoutUser, UserProfile } from '../services/firebase';
...
const Profile: React.FC = () => {
  ...
  // 2. Format date utility
  const formatDate = (timestamp: any): string => {
    if (!timestamp) return 'N/A';
    if (
      typeof timestamp === 'object' &&
      ('isEqual' in timestamp || '_methodName' in timestamp)
    ) {
      return 'N/A';
    }
    let date: Date;
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        <input
                          ...
                          className="w-full px-4 py-3 rounded-xl border-0 bg-indigo-50/80 placeholder-neutral-600 text-neutral-800 focus:bg-indigo-100 focus:ring-2 focus:ring-primary-400 focus:outline-none transition-all duration-300"
                  ...
                  {isEditing && (
                    <div className="flex items-center gap-4 mt-8 pt-6 border-t border-white/20">
                      <button
                        onClick={handleSave}
                        disabled={isSaving}
                        className="flex items-center gap-2 bg-gradient-to-r from-red-400 to-pink-400 text-white font-medium py-2 px-4 rounded-xl hover:from-red-500 hover:to-pink-500 transform hover:scale-105 focus:ring-4 focus:ring-red-200 focus:outline-none transition-all duration-300 shadow-lg"
                      >
                        {isSaving ? (
                          <>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-neutral-800">
                        {Math.floor((profile?.studyStats?.totalStudyTime || 0) / 60)}h {(profile?.studyStats?.totalStudyTime || 0) % 60}m
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-neutral-600">Sessions Completed</span>
                      <span className="font-medium text-neutral-800">{profile?.studyStats?.sessionsCompleted || 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-neutral-600">Quizzes Completed</span>
                      <span className="font-medium text-neutral-800">{profile?.studyStats?.quizzesCompleted || 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-neutral-600">Average Score</span>
                      <span className="font-medium text-neutral-800">{profile?.studyStats?.averageScore || 0}%</span>
                    </div>
                  </div>
                </div>
                {/* Quick Actions */}
                <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-6 shadow-xl">
                  <h3 className="text-lg font-serif font-bold text-neutral-800 mb-6 flex items-center gap-2">
